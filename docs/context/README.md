...
- A Go usage collector under `/agent/cmd/fabric-collector` that leases the data plane's durable
  spool and forwards usage under a separate write-only credential; the control plane accepts it
  on `/v1/telemetry/usage`.
