...
- The data plane tracks **per-backend health** and selects a healthy backend for each request. When a backend's connection fails it is marked unhealthy and, for a non-streamed request, another healthy backend is chosen so killing one replica does not fail requests. A backend recovers after a cooling interval.
