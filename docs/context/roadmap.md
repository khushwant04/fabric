...
- Versioned `FabricModelDeployment` CRD and cluster-local operator with conditions/rollback. **Partly implemented**: the CRD and operator ship and are verified on a real cluster; the operator renders the data plane's configuration and reports conditions, but creates no model-host workload and has no rollback.
