...
Verified locally: `ruff` reports no findings, 154 tests pass on SQLite and 165 on PostgreSQL against a database built by the real Alembic migration, `alembic upgrade → downgrade → upgrade` reproduces the application tables, and `openapi.json` matches the running app, which a test enforces. The eleven extra PostgreSQL tests are the row-level-security suite, which SQLite cannot represent and therefore skips rather than passing vacuously.
